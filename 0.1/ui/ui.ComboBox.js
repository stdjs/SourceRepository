        ...
        */
        initEvents:function(){
            ...
            that[0].on({
                ...
                focusin:function(e){
                    that.addClass("focus").emit("focus",e);
                    if(state === false){
                        that.initKeyboard().on("focusout",function(e){
                            (that.emit("blur",e))[0].removeClass("focus");
                        });
    ...
    public:{
        ...
        */
        insert:function(data,index){
            ...
            if(isWidget(item)){
                var items    = that.items;
                var listElem = that.D.list;

                if(listElem){
                    if(index < items.length){
                        listElem.insertBefore(item,items[index]);
                    }else{
